...
|---|---|---|---|---|
| `v1.9.0` | 1.9.0 |  |  | **MINOR: `speech-upscale` routes to an on-box door, so no local speech audio reaches RunPod (#383/#384).** `LOCAL_FINISH_SPEECH_URL`; the generic half of the #378 door pool extracted to `door-pool.ts` and genuinely SHARED, with the old names re-exported as aliases so contracts and import paths are unchanged. **The key wins on PRESENCE, not usability** -- its fall-through is a CLOUD CALL rather than a refusal, so a typo read as 'unset' would silently restore the traffic it exists to remove. **This release is what makes the key settable at all:** #384 merged after the v1.8.0 cut, and a studio on 1.8.0 answers `PATCH /api/settings/secrets` with `ok:true, applied:[]` for an unknown key rather than refusing. Also retracts an overstated #380 note (#385). |
| `v1.8.0` | 1.8.0 |  |  | **MINOR: the finish backend takes a door POOL, not one door (#378).** `LOCAL_FINISH_*_URL` accepts a comma-separated list; a single value is bit-for-bit unaffected and takes no health probe at all. Rotation on submit, **affinity on poll** (the poll token records the serving door) -- an async door with per-container in-memory job state is why a load balancer could not do this. **Keys live in the platform runtime store, NOT `.env` (#379); setting the list in `.env` alone has no effect.** Also repairs a duplicated v1.7.0 section in CHANGELOG.md. **Local-only by construction:** vivijure-cf has no local-finish path (vivijure-cf#480), so no cf half this wave. |
